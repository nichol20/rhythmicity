...
SELECT genre FROM albums_genres ag INNER JOIN genres g ON ag.genreId = g.id WHERE ag.id = $1;

-- name: GetAlbumStyles :many
SELECT style FROM albums_styles ast INNER JOIN styles s ON ast.styleId = s.id WHERE ast.id = $1;
