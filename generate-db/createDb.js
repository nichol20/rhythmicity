...
const fetchData = async track => {
    ...
    const genreObj = await googleCustomSearch.getGenresAndStyles({ albumName: albumData.name, artist: artistNames[0]})
...
const createData = async ({ albumData, artistsData, videoData, lyrics, genreObj, track }) => {
    ...
    if('_exists' in albumData) {
        ...
        // create album record
        albumId = uuidv4()
        db.addAlbum({ album: albumData, albumId, trackId, artistIds, genres: genreObj.genres, styles: genreObj.styles })
    }
